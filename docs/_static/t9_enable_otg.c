...
int main(void) {
    ...
    if ((ret = ioctl(i2cFile, I2C_SLAVE_FORCE, 0x36)) < 0) printf("ioctl failed: %d\n", ret);
    printf("ioctl init done\n");

    ret = max77818_get_i2c_regU16(i2cFile, 0x36, 0x02, &valU16, &lsbU8, &msbU8);
    printf("get done\n");

    printf("Read temp alert Register %d ret = %d, valU16=%d, lsbU8=%d, msbU8=%d\n", 0x02, ret, valU16, lsbU8, msbU8);
